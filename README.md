...
| `tsconfig.json` | TypeScript build config (rootDir `.` → `outDir dist/`) |
| `dist/lib/sync/clock.js` | Compiled clock module (generated by `npm run build`) |
...
```bash
npm install
npm run build     # tsc -p tsconfig.json (-> dist/) + generates public/rr-config.js
npm start         # http://localhost:3000
```

Same-origin mode needs **no environment variables** — the Node server serves
the frontend and the WebSocket on one port. The compiled clock module is read
from `dist/lib/sync/clock.js` and served at the client's `/lib/sync/clock.js`
URL (and copied into `public/lib/` for static-only hosts like Vercel).

> Note: `npm run build` must run before `npm start` — the server warns if
> `dist/lib/sync/clock.js` is missing. The TypeScript version is pinned
> **exactly** (`5.9.3`) and Node engines to `>=24 <25` in `package.json`, so
> builds are reproducible on Vercel and anywhere else.
...
```bash
npm test          # clock module unit tests (node --test, native type stripping)
npm run build     # tsc -p tsconfig.json (compiles lib/sync/clock.ts -> dist/) + gen-config
npm run e2e       # browser E2E: cross-device sync + Sounds of Azaadi + routing (needs local Chrome)
```
